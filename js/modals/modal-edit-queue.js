...
async function openEditModal(queue) {
    ...
    formHTML += `</form>
                 <div class="modal-actions">
                    <button class="btn btn-secondary" onclick="closeModal()">Cancel</button> <button class="btn btn-primary" onclick="stageQueueChanges()">Stage Changes</button>
                 </div>`;
        ...
        capacityModeSelect.addEventListener('change', () => {
            ...
            for (const category of QUEUE_CONFIG_CATEGORIES) {
                for (const phPropName in category.properties) {
                    if (phPropName.endsWith('.capacity') && phPropName.includes(Q_PATH_PLACEHOLDER)) {
            ...
            if (capacityInput && capacityPropDef) {
                ...
                const helpTextElement = capacityInput.closest('.form-group.property-edit-item')?.querySelector('.form-help');
                if(helpTextElement) { // This was for the main description, but we can append mode info
...
function stageQueueChanges() {
    ...
    if (newCapacityMode !== originalCapacityMode) {
        ...
        uiHints['_ui_capacityMode'] = newCapacityMode; 
        hasDetectedChanges = true; 
    } else if (existingPendingChanges['_ui_capacityMode']) { // Preserve if not changed this session
    ...
    form.querySelectorAll('input.form-input, select.form-input').forEach(inputElement => {
        ...
        if (newValue !== originalValue) {
            ...
            for (const category of QUEUE_CONFIG_CATEGORIES) {
                const ph = Object.keys(category.properties).find(p => p.replace(Q_PATH_PLACEHOLDER, queuePath) === actualPropName);
                if (ph) { propDefForType = category.properties[ph]; break; }
            }

            if (actualPropName.endsWith('.capacity')) {
                if (effectiveCapacityMode === 'percentage' && !newValue.endsWith('%')) newChangesForAPI[actualPropName] = (parseFloat(newValue) || 0).toFixed(1) + '%';
                else if (effectiveCapacityMode === 'weight' && !newValue.endsWith('w')) newChangesForAPI[actualPropName] = (parseFloat(newValue) || 0).toFixed(1) + 'w';
                else if (effectiveCapacityMode === 'absolute' && !(newValue.startsWith('[') && newValue.endsWith(']'))) newChangesForAPI[actualPropName] = `[${newValue.replace(/[\[\]]/g, '')}]`; // Basic wrap
    ...
    if(Object.keys(newChangesForAPI).length > 0) {
        for(const key in newChangesForAPI) finalChangesToStore[key] = newChangesForAPI[key];
        storeNeeded = true;
    }
    if(uiHints['_ui_capacityMode'] && uiHints['_ui_capacityMode'] !== existingPendingChanges['_ui_capacityMode']){
        finalChangesToStore['_ui_capacityMode'] = uiHints['_ui_capacityMode'];
        storeNeeded = true;
    } else if (uiHints['_ui_capacityMode']) { // Preserve if it was already set and not changed this session
        finalChangesToStore['_ui_capacityMode'] = uiHints['_ui_capacityMode'];
    }


    if (storeNeeded || hasDetectedChanges) { // hasDetectedChanges covers initial mode change
        pendingChanges.set(queuePath, finalChangesToStore);
        console.log("Staged changes for", queuePath, ":", finalChangesToStore);
        if (typeof showSuccess === "function") showSuccess(`Changes staged for queue "${currentEditQueue.name}"`);
    } else {
        if (typeof showInfo === "function") showInfo("No new changes detected to stage.");
    }
