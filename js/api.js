    ...
    class YarnSchedulerAPI {
        ...
        async makeConfigurationUpdateApiCall({deletions = [], additions = [], updates = [], globalUpdates = []}) {
            const xmlBody = this.buildBatchMutationXML({deletions, additions, updates, globalUpdates});
            const options = {
                headers: {
            ...
            const response = await this.makeRequest(API_ENDPOINTS.SCHEDULER_CONF,  options);
        ...
        buildBatchMutationXML({deletions = [], additions = [], updates = [], globalUpdates = []}) {
            ...
            additions.forEach(newQueue => {
                ...
                xmlParts.push(`
                    <add-queue>
                        <queue-name>${escapeXml(newQueue.queueName)}</queue-name>
                        ${params}
                    </add-queue>`);
            });
            ...
            updates.forEach(update => {
                ...
                xmlParts.push(`
                    <update-queue>
                        <queue-name>${escapeXml(update.queueName)}</queue-name>${params}
                    </update-queue>`);
            });
