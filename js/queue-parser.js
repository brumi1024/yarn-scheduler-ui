...
function determineQueueType(queueInfo) {
    ...
    const hasChildren = queueInfo.queues?.queue &&
...
function parseSchedulerData(schedulerInfo) {
    ...
    function parseQueue(queueInfo, parentPath = '') {
        ...
        const queuePath = parentPath ? `${parentPath}.${queueInfo.queueName}` : queueInfo.queueName;
        ...
        const queue = {
            ...
            weight: parseFloat(queueInfo.weight) || parseFloat(queueInfo.normalizedWeight) || defaults.weight,
            normalizedWeight: parseFloat(queueInfo.normalizedWeight) || defaults.normalizedWeight,
            ...
            absoluteCapacity: parseFloat(queueInfo.absoluteCapacity) || defaults.absoluteCapacity,
            absoluteMaxCapacity: parseFloat(queueInfo.absoluteMaxCapacity) || defaults.absoluteMaxCapacity,
            effectiveCapacity: parseFloat(queueInfo.effectiveCapacity || queueInfo.capacity) || defaults.effectiveCapacity,
            effectiveMaxCapacity: parseFloat(queueInfo.effectiveMaxCapacity || queueInfo.maxCapacity) || defaults.effectiveMaxCapacity,
            state: queueInfo.state || defaults.state,
            userLimitFactor: parseFloat(queueInfo.userLimitFactor) || defaults.userLimitFactor,
            maxApplications: parseInt(queueInfo.maxApplications) || defaults.maxApplications,
            numApplications: parseInt(queueInfo.numApplications) || defaults.numApplications,
            nodeLabels: Array.isArray(queueInfo.nodeLabels) ? queueInfo.nodeLabels : defaults.nodeLabels,
            defaultNodeLabelExpression: queueInfo.defaultNodeLabelExpression || defaults.defaultNodeLabelExpression,
            autoCreationEligibility: queueInfo.autoCreationEligibility || defaults.autoCreationEligibility,
