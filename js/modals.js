...
function addNewQueue() {
    ...
    const capacityValue = document.getElementById('new-queue-capacity').value.trim();
    const maxCapacity = parseFloat(document.getElementById('new-queue-max-capacity').value);
    ...
    if (capacityMode === 'weight') {
        parsedCapacity = capacityValue.endsWith('w') ? capacityValue : `${capacityValue}w`;
...
function stageQueueChanges() {
    ...
    const maxCapacity = parseFloat(document.getElementById('queue-max-capacity').value);
    ...
    // Check capacity
    if (capacityMode === 'weight') {
        const weightValue = capacityValue.endsWith('w') ? capacityValue : `${capacityValue}w`;
        if (weightValue !== (currentEditQueue.weight ? `${currentEditQueue.weight}w` : currentEditQueue.capacity)) {
...
async function applyAllChanges() {
    ...
    const additions = Array.from(pendingAdditions.values()).map(newQueue => {
        ...
        if (newQueue.maxCapacity !== undefined) params["maximum-capacity"] = newQueue.maxCapacity;
    ...
    const updates = Array.from(pendingChanges.entries()).map(([queuePath, changes]) => {
        const params = {};
    ...
    try {
        ...
        if (response && response.status == 200 && typeof response.data === 'string' && response.data.includes('Configuration change successfully applied.')) {
            ...
            if (response && response.data) {
                if (typeof response.data === 'string') {
                    ...
                    } else if (response.data.includes('error') || response.data.includes('failed')) {
...
function markQueueForDeletion(queuePath) {
    ...
    if (hasChildren || hasNewChildren) {
        ...
        const newChildren = Array.from(pendingAdditions.values()).filter(newQueue =>
            newQueue.parentPath === queuePath
        );

        // Check if all existing children are marked for deletion
        const allExistingChildrenMarkedForDeletion = existingChildren.every(child =>
            pendingDeletions.has(child.path)
        );

        // If there are new children that aren't being deleted, we can't delete the parent
        if (newChildren.length > 0) {
            showWarning('Cannot delete queue with pending new child queues. Please remove the new child queues first.');
            return;
        }
        ...
        if (!allExistingChildrenMarkedForDeletion) {
            ...
            if (activeChildren.length === 1) {
                showWarning(`Cannot delete queue with active child queue "${activeChildren[0].name}". Please delete the child queue first.`);
            } else {
                const childNames = activeChildren.map(child => child.name).join('", "');
                showWarning(`Cannot delete queue with active child queues: "${childNames}". Please delete all child queues first.`);
            }
            return;
        }
        ...
        const confirmMessage = `Are you sure you want to delete queue "${queue.name}" and its ${childCount} child queue${childCount > 1 ? 's' : ''}: "${childNames}"?\n\nAll child queues are already marked for deletion.`;
...
function openEditModal(queue) {
    ...
    document.getElementById('modal-title').textContent = `Edit Queue: ${queue.name}`;
    document.getElementById('queue-name').value = queue.name;

    // Set capacity mode
    const capacityMode = pendingChange?.capacityMode || queue.capacityMode || 'percentage';
    ...
    document.getElementById('queue-max-capacity').value = pendingChange?.maxCapacity || queue.maxCapacity;
    document.getElementById('queue-state').value = pendingChange?.state || queue.state;
