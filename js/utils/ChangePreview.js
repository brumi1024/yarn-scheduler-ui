...
class ChangePreview {
    ...
    _findLongestValidQueuePath(currentPath, remainingParts, queueData) {
        ...
        const childQueueNames = queueInfo.queues.queue
            ? Array.isArray(queueInfo.queues.queue)
                ? queueInfo.queues.queue.map((q) => q.queueName)
                : [queueInfo.queues.queue.queueName]
            : [];
    ...
    _findQueueInHierarchy(queuePath, queueData) {
        ...
        for (let i = 0; i < pathParts.length; i++) {
            ...
            if (currentQueue.queues && currentQueue.queues.queue) {
                const queues = Array.isArray(currentQueue.queues.queue)
    ...
    static fromChangeLog(changeLog) {
        ...
        for (const change of allChanges) {
            ...
            if (
                change.operation === OPERATION_TYPES.UPDATE &&
                change.queuePath &&
                change.propertyKey &&
