...
import { X, Save, RotateCcw, GitBranch, Info, Settings, Edit, AlertCircle, ChevronDown, ChevronUp } from 'lucide-react';
                    ...
                    <div className="flex flex-col h-full relative overflow-hidden">
                        <SheetHeader>
                            <div className="flex items-center justify-between">
                                <SheetTitle className="text-base font-semibold">Queue: {selectedQueue.queueName}</SheetTitle>
                            </div>
                            {/* Status bar */}
                                    ...
                                    <div className="pb-2 px-2 space-y-1">
                                        ...
                                        {Object.entries(validationErrors).map(([field, error]) => (
                                            <div key={field} className="text-xs text-muted-foreground pl-2">
                                                • <span className="font-medium">{field}:</span> {typeof error === 'object' && error && 'message' in error ? (error as any).message : String(error)}
                                            </div>
                                        ))}
                                    </div>
                                )}
