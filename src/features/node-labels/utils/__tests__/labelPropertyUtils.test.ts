...
describe('labelPropertyUtils', () => {
    ...
    it('should generate capacity, maximum-capacity, and maximum-am-resource-percent properties for each label', () => {
      ...
      labelNames.forEach((label) => {
        propertyTypes.forEach((propertyType) => {
          const property = labelProperties.find(
            (p) => p.label === label && p.basePropertyName === propertyType,
          );
  ...
  describe('extractLabelFromPropertyName', () => {
    it('should extract label name from property name', () => {
      ...
      expect(extractLabelFromPropertyName('accessible-node-labels.cpu.maximum-capacity')).toBe(
        'cpu',
      );
  ...
  describe('extractBasePropertyFromLabelProperty', () => {
    it('should extract base property name from label property', () => {
      expect(extractBasePropertyFromLabelProperty('accessible-node-labels.gpu.capacity')).toBe(
        'capacity',
      );
      expect(
        extractBasePropertyFromLabelProperty('accessible-node-labels.cpu.maximum-capacity'),
      ).toBe('maximum-capacity');
