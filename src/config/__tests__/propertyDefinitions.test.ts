...
describe('propertyDefinitions', () => {
    describe('queuePropertyDefinitions', () => {
        ...
        it('has validation rules for properties that need them', () => {
            ...
            const userLimitProperty = queuePropertyDefinitions.find(p => p.name === 'minimum-user-limit-percent');
    ...
    describe('globalPropertyDefinitions', () => {
        ...
        it('has correct enum values for resource calculator', () => {
            const resourceCalcProperty = globalPropertyDefinitions.find(p => p.name === 'resource-calculator');
            expect(resourceCalcProperty?.type).toBe('enum');
            expect(resourceCalcProperty?.enumValues).toContain('org.apache.hadoop.yarn.util.resource.DefaultResourceCalculator');
            expect(resourceCalcProperty?.enumValues).toContain('org.apache.hadoop.yarn.util.resource.DominantResourceCalculator');
        });
    });
    ...
    describe('property completeness', () => {
        ...
        it('has properties for preemption control', () => {
            const preemptionProperties = queuePropertyDefinitions.filter(
                p => p.name.includes('preemption')
            );
            expect(preemptionProperties.length).toBeGreaterThan(0);
        });

        it('has properties for application lifetime management', () => {
            const lifetimeProperties = queuePropertyDefinitions.filter(
                p => p.name.includes('lifetime')
            );
