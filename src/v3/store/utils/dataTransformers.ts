 ...
 */
export const extractQueueConfig = (
  ...
  const childQueueNames = fullConfig[queuesKey]
    ? fullConfig[queuesKey].split(',').map(s => s.trim())
    : [];
...
export const buildQueueTree = (
  ...
  const buildNode = (
    queuePath: string,
    queueName: string,
    queueMetrics?: SchedulerResponse
  ): QueueNode => {
    ...
    if (queuesStr) {
      ...
      childNames.forEach(childName => {
        ...
        if (queueMetrics?.queues?.queue) {
          childMetrics = queueMetrics.queues.queue.find(
            q => q.queueName === childName
          );
        }
