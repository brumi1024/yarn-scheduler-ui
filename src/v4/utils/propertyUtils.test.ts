...
describe('propertyUtils', () => {
    ...
    describe('buildNodeLabelPropertyKey', () => {
        ...
        it('should build node label property key for nested queue', () => {
            ...
            expect(key).toBe('yarn.scheduler.capacity.root.production.team1.accessible-node-labels.ssd.maximum-capacity');
        });

        it('should build node label property key with complex property', () => {
            const key = buildNodeLabelPropertyKey('root.production', 'gpu', 'maximum-am-resource-percent');
            expect(key).toBe('yarn.scheduler.capacity.root.production.accessible-node-labels.gpu.maximum-am-resource-percent');
        });
    ...
    describe('property key analysis utilities', () => {
        describe('isGlobalPropertyKey', () => {
            ...
            it('should return false for queue property keys', () => {
                ...
                expect(isGlobalPropertyKey('yarn.scheduler.capacity.root.production.team1.state')).toBe(false);
            });

            it('should return false for node label property keys', () => {
                expect(isGlobalPropertyKey('yarn.scheduler.capacity.root.default.accessible-node-labels.gpu.capacity')).toBe(false);
            });
    ...
    describe('edge cases and error handling', () => {
        ...
        it('should handle node labels with special characters', () => {
            ...
            expect(key).toBe('yarn.scheduler.capacity.root.default.accessible-node-labels.gpu-high_mem.capacity');
        });
