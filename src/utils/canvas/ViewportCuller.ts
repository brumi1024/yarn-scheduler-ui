...
export class ViewportCuller {
    ...
    private isNodeInViewport(node: LayoutNode, viewport: Viewport): boolean {
        return this.rectIntersectsViewport(node.x, node.y, node.width, node.height, viewport);
    }
    ...
    private isFlowInViewport(flow: FlowPath, viewport: Viewport): boolean {
        ...
        return this.isNodeInViewport(flow.source, viewport) || this.isNodeInViewport(flow.target, viewport);
    }

    /**
     * Check if rectangle intersects viewport
     */
    private rectIntersectsViewport(x: number, y: number, width: number, height: number, viewport: Viewport): boolean {
        return !(
            x + width < viewport.x ||
