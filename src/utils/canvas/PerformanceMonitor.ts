...
export class PerformanceMonitor {
  ...
  private checkPerformanceThresholds(fps: number, frameTime: number, memoryUsage: number): void {
    ...
    if (frameTime > this.FRAME_TIME_ERROR_THRESHOLD) {
      this.addAlert('error', `High frame time: ${frameTime.toFixed(1)}ms`, frameTime, this.FRAME_TIME_ERROR_THRESHOLD, now);
    } else if (frameTime > this.FRAME_TIME_WARNING_THRESHOLD) {
      this.addAlert('warning', `Elevated frame time: ${frameTime.toFixed(1)}ms`, frameTime, this.FRAME_TIME_WARNING_THRESHOLD, now);
    }

    // Memory checks
    if (memoryUsage > this.MEMORY_ERROR_THRESHOLD) {
      this.addAlert('error', `High memory usage: ${(memoryUsage / 1024 / 1024).toFixed(1)}MB`, memoryUsage, this.MEMORY_ERROR_THRESHOLD, now);
    } else if (memoryUsage > this.MEMORY_WARNING_THRESHOLD) {
      this.addAlert('warning', `Elevated memory usage: ${(memoryUsage / 1024 / 1024).toFixed(1)}MB`, memoryUsage, this.MEMORY_WARNING_THRESHOLD, now);
    }
  }
   ...
   */
  private addAlert(type: 'warning' | 'error', message: string, value: number, threshold: number, timestamp: number): void {
