...
export const validateChildCapacitySum: BusinessValidator = (_, context) => {
  ...
  for (const child of queue.queues.queue) {
    const capacity = context.configData.get(
      `yarn.scheduler.capacity.${child.queuePath}.capacity`
    );
...
export const validateMaxCapacityRelationship: BusinessValidator<string> = (maxCapacity, context) => {
  if (!maxCapacity || maxCapacity.trim() === '' || maxCapacity === '-1') {
    return { valid: true, errors: [] };
  }

  const capacity = context.configData.get(
    `yarn.scheduler.capacity.${context.queuePath}.capacity`
  );
...
export const validateParentChildCapacityConstraints: BusinessValidator<string> = (value, context) => {
