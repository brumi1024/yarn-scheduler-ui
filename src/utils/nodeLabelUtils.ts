...
export function hasQueueAccessToLabel(queue: Queue, label: string, allQueues?: Queue[]): boolean {
    ...
    const accessibleLabels = allQueues ? getQueueAccessibleLabels(queue, allQueues) : null;
...
export function getInheritanceTooltip(
    ...
    if (isCapacityInherited || isMaxCapacityInherited) {
        parts.push(
            `(No specific ${isCapacityInherited && isMaxCapacityInherited ? 'capacities' : isCapacityInherited ? 'capacity' : 'max capacity'} configured for ${label} label)`
        );
    }
