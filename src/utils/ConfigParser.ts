...
export class ConfigParser {
  ...
  static parse(config: Configuration): ParseResult {
    ...
    try {
      ...
      result.errors.push(`Configuration parsing failed: ${error instanceof Error ? error.message : String(error)}`);
    }
      ...
      for (const [key, value] of Object.entries(config)) {
        ...
        if (suffix.endsWith('.queues')) {
          ...
          if (queuePaths.has(parentPath)) {
            ...
            const childNames = value.split(',').map((s: string) => s.trim()).filter((s: string) => s.length > 0);
  ...
  private static buildQueueHierarchy(properties: ParsedProperty[], result: ParseResult): void {
    ...
    if (queuePropertyMap.has(this.ROOT_QUEUE) || 
    ...
    for (const prop of queueProperties) {
      if (prop.property === 'queues' && prop.value) {
        ...
        const childNames = prop.value.split(',').map((s: string) => s.trim()).filter((s: string) => s.length > 0);
        for (const childName of childNames) {
          const childPath = `${parentPath}.${childName}`;
    ...
    for (const queue of queueMap.values()) {
      ...
      if (queuesProperty) {
        const childNames = queuesProperty.split(',').map((s: string) => s.trim()).filter((s: string) => s.length > 0);
        ...
        for (const childName of childNames) {
          ...
          if (childQueue) {
            ...
            result.warnings.push(`Child queue '${childName}' declared in '${queue.path}' but not found in configuration`);
          }
        }
      }
    }
  ...
  private static createQueueFromProperties(
    ...
    const queue: ParsedQueue = {
      ...
      intraQueuePreemptionDisabled: this.parseBooleanProperty(properties.get('intra-queue-preemption.disable_preemption'), false),
