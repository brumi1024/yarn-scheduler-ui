...
function validateQueueHierarchy(queue: ParsedQueue, errors: ValidationError[], warnings: ValidationError[]): void {
    ...
    if (queue.maxAMResourcePercent !== undefined && (queue.maxAMResourcePercent < 0 || queue.maxAMResourcePercent > 1)) {
        errors.push({
    ...
    if (queue.minimumUserLimitPercent !== undefined && (queue.minimumUserLimitPercent <= 0 || queue.minimumUserLimitPercent > 100)) {
        errors.push({
...
function validateLegacyModeConstraints(queue: ParsedQueue, errors: ValidationError[], warnings: ValidationError[]): void {
...
function validateGlobalProperties(globalProps: Record<string, string>, errors: ValidationError[], warnings: ValidationError[]): void {
    // Validate resource calculator setting
    const resourceCalculator = globalProps['yarn.scheduler.capacity.resource-calculator'];
    if (resourceCalculator && !['DefaultResourceCalculator', 'DominantResourceCalculator'].includes(resourceCalculator)) {
        warnings.push({
...
export function validateConfigurationChange(
    ...
    // Create a new config with the proposed change
