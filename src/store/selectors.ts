...
export const selectHasUnsavedChanges = (state: RootState) => 
  state.stagedChanges.changes.length > 0;
...
export const selectIsQueueExpanded = (state: RootState, queuePath: string) =>
  state.ui.expandedQueues.has(queuePath);
...
export const selectRecentLogs = (state: RootState, count: number = 10) =>
...
const memoizeOne = <Args extends unknown[], Return>(
  fn: (...args: Args) => Return
): ((...args: Args) => Return) => {
