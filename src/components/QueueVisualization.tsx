...
export const QueueVisualization: React.FC<QueueVisualizationProps> = ({ className }) => {
    ...
    // Handle queue properties save
    const handleQueueSaveProperties = useCallback((queuePath: string, changes: Record<string, any>) => {
        // TODO: Save queue properties via API
        console.log('Saving properties for queue:', queuePath, changes);
    }, []);

    // Handle queue selection from child queue clicks
    const handleQueueSelect = useCallback((queue: Queue) => {
        ...
        if (nodeToSelect && selectionController) {
            // Trigger selection through the selection controller to maintain consistency
            selectionController.selectNode(nodeToSelect.id);
        } else {
            // Fallback: directly update state if node not found or no selection controller
            ...
            if (renderer) {
                const selectedSet = new Set([queue.queueName]);
                renderer.setSelectedNodes(selectedSet);
                const state = panZoomController?.getState() || { x: 0, y: 0, scale: 1 };
                renderer.render(nodes, flows, state);
            }
        }
    }, [nodes, selectionController, renderer, panZoomController]);

    // Handle API error
    if (apiError) {
        return (
            <Box sx={{ p: 3 }}>
                <Alert severity="error">Failed to load scheduler data: {apiError?.message || 'Unknown error'}</Alert>
            </Box>
        );
    }
