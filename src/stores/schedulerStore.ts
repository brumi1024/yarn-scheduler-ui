  ...
  immer<SchedulerStore>((set, get) => ({
    ...
    stageQueueAddition: (parentPath, queueName, config) => {
      ...
      if (!config || Object.keys(config).length === 0) {
        throw createStoreError(ERROR_CODES.VALIDATION_ERROR, 'Queue configuration cannot be empty');
      }
    ...
    applyChanges: async () => {
      ...
      if (changes.length === 0) {
        throw createStoreError(ERROR_CODES.EMPTY_STAGED_CHANGES, 'No staged changes to apply');
      }
      ...
      try {
        ...
        const errorMessage = createDetailedErrorMessage('apply changes', error, {
        ...
        throw createStoreError(ERROR_CODES.APPLY_CHANGES_FAILED, errorMessage, error);
      }
    },
    ...
    getLabelChangesForQueue: (queuePath, label) => {
      ...
      return state.stagedChanges.filter(
        (c) =>
          c.queuePath === queuePath && c.property?.includes(`accessible-node-labels.${label}.`),
      );
    },
