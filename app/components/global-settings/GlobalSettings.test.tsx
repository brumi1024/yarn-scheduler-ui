/**
 * Tests for GlobalSettings component
 * 
 * This component manages global scheduler settings, displaying them in categorized
 * accordions and allowing users to modify values. Changes are staged and must be
 * applied to take effect.
 * 
 * Test coverage includes:
 * - Rendering with/without properties
 * - Category organization and display
 * - Property value display and updates
 * - Staged changes tracking and alerts
 * - User interactions (changing values)
...
import type { StagedChange } from '../../lib/types/staged-change';

// Mock the store
vi.mock('~/store/schedulerStore');

// Mock the property definitions
vi.mock('~/lib/config/propertyDefinitions', () => ({
...
describe('GlobalSettings', () => {
    ...
    describe('unsaved changes alert', () => {
        ...
        it('should display alert when there are global staged changes', () => {
            ...
            expect(screen.getByText('You have 2 unsaved global settings. Apply changes to make them active.')).toBeInTheDocument();
        });
        ...
        it('should display correct singular/plural text for staged changes', () => {
            ...
            expect(screen.getByText('You have 1 unsaved global setting. Apply changes to make them active.')).toBeInTheDocument();
        });
    ...
    describe('user interactions', () => {
        it('should call stageGlobalChange when property value is changed', () => {
            const properties = [
                getMockPropertyDescriptor({ name: 'test-property' })
            ];
            (globalPropertyDefinitions as PropertyDescriptor[]).push(...properties);
