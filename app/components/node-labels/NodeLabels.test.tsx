...
vi.mock('./NodesPanel', () => ({
    NodesPanel: ({ selectedLabel }: { selectedLabel: string | null }) => (
        <div data-testid="nodes-panel">
            Nodes Panel - Selected: {selectedLabel || 'none'}
        </div>
    )
}));
...
describe('NodeLabels', () => {
    ...
    describe('Panel layout', () => {
        ...
        it('should display card descriptions', () => {
            ...
            expect(screen.getByText('Select labels to configure queue capacity for each label')).toBeInTheDocument();
            expect(screen.getByText('Assign nodes to labels for resource allocation')).toBeInTheDocument();
        });
    });
